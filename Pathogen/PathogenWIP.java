...
{
	public static HashSet<String> correctPaths = new HashSet<String>();
	...
	{
		return Pathogen.correctPaths;
	}
	public static boolean solveMaze(char [][] maze)
	{
		//HashSet<String> correctPaths = new HashSet<String>();
		String path = "";
		...
		// Let's goooooooo!!
		return solveMaze(maze, visited, startRow, startCol, height, width, path);
	}

	private static boolean solveMaze(char [][] maze, char [][] visited,
	                                 int currentRow, int currentCol,
	                                 int height, int width, String path)
	{
		...
		{
			Pathogen.correctPaths.add(path);
			System.out.println("RETURN HASH SET ");
		...
		for (int i = 0; i < moves.length; i++)
		{
			...
			if (!isLegalMove(maze, visited, newRow, newCol, height, width))
				continue;
//----------------------------------------------------------------------------
//WHERE I SHOULD BE ADDING TO STRING BUILDER :d
			...
			//System.out.println("BEFORE MOVE" + path);
			path = fasterConcatenation(path, i);
			System.out.println(path);
			//System.out.println("AFTER MOVE" + path);
			...
			if (solveMaze(maze, visited, newRow, newCol, height, width, path))
				return true;
			...
			//System.out.println("BEFORE REMOVE" + path);
			//fasterDeletion(path);
			...
			System.out.println("WE ARE BACKTRACKING BABY");
			System.out.println(path);
			if (Pathogen.animationEnabled)
			{
	...
	{
		if (maze[row][col] == WALL || maze[row][col] == CORONA || visited[row][col] == BREADCRUMB) //|| row >= width || col >= height)
			return false;
	...
	// stringbuilder instead of string concat :DD
	public static String fasterConcatenation(String s, int i)
	{
		...
		if (i == 0)
		{
			...
			builder.append(s).append('l').append(" ");
		}
	 	if (i == 1)
		{
			System.out.println("moving right");
 		 	builder.append(s).append('r').append(" ");
		}
	 	if (i == 2)
		{
			System.out.println("moving up");
 		 	builder.append(s).append('u').append(" ");
		}
	 	if (i == 3)
		{
			System.out.println("moving down");
 		 	builder.append(s).append('d').append(" ");
		}
		//System.out.println("CURRENT PATH" + builder.toString());
		//System.out.println("CURRENT PATH" + s);
		return builder.toString();
	}

	public static String fasterDeletion(String s)
	{
		StringBuilder builder = new StringBuilder(s.length());

		builder.setLength(s.length() - 1);
		return builder.toString();
	}
	...
	public static void main(String [] args) throws IOException
	{
		...
		char [][] maze = readMaze("mazeTest2.txt");
		...
		if (Pathogen.solveMaze(maze))
		{
			...
			System.out.println(Pathogen.correctPaths);
		}
		else
