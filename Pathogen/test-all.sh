...
if [ ! -f Pathogen.java ]; then
	echo ""
	echo " Error: You must place Pathogen.java in this directory"
...
do
	...
	javac Pathogen.java TestCase$i.java 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " Pathogen.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" Pathogen.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" Pathogen.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" Pathogen.java -n"
		echo "     grep \"\$(printf '^\t')\" Pathogen.java -n"
